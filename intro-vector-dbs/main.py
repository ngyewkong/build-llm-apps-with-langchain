...
def format_docs(docs):
    # to append the retrieved documents into one doc
...
if __name__ == "__main__":
    ...
    # taking all the documents and stuff them into the chain
    combined_docs_chain = create_stuff_documents_chain(
        llm, retrieval_qa_chat_prompt)

    retrieval_chain = create_retrieval_chain(
        retriever=vectorstore.as_retriever(), combine_docs_chain=combined_docs_chain
    )
    ...
    rag_chain = (
        {"context": vectorstore.as_retriever() | format_docs,
         "question": RunnablePassthrough()}  # Question will propagate to the final llm call
        | custom_rag_prompt
        | llm
    )
    ...
    print(f"rag chain response: {res}")
